...
public class ActionReceiver extends BroadcastReceiver {
    @Override public void onReceive(Context c,Intent i){
        ...
        final PendingResult pending=goAsync();Executors.newSingleThreadExecutor().execute(()->{try{JSONObject response=Api.action(store.credential(),id,action,minutes);if(response.has("state")){store.state(response.getJSONObject("state").toString());store.revision(response.optLong("revision",store.revision()));AlarmScheduler.reschedule(c);}}catch(Exception e){try{JSONArray q=store.pendingActions();q.put(new JSONObject().put("id",id).put("action",action).put("minutes",minutes));store.pendingActions(q);}catch(Exception ignored){}}finally{pending.finish();}});
    }
